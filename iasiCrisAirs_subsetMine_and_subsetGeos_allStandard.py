...
rttovPath = "/discover/nobackup/bkarpowi/rt3/rttov/"
import matplotlib
matplotlib.use('Agg')
import numpy as np
import os
...
def plotWeightingFunctions(chan_list, profiles, wf, instrument, profileNumber, wavenumbers):
    ...
    if(len(chan_list)>30): matplotlib.rc('xtick', labelsize=4) 
    ...
    plt.pcolor(np.arange(wf.shape[1]+1), myProfiles.P[int(profileNumber)-1,0:99], wf[0:99,:],vmin=wf[0:99,:].min(), vmax= 0.5)#vmax = wf[:,0:99].max() ) )
    plt.colorbar()
    plt.ylabel('Pressure [hPa]')
    ...
    plt.yticks(np.array([1000.0, 100.0, 10.0, 1.0, 0.1]),['1000.0','100.0','10.0','1.0','0.1'])
    plt.tight_layout() 
    plt.savefig(instrument+'_profile_'+profileNumber+'_weighting_functions_pcolor.pdf')
    plt.close()
...
def plotJacobians(chan_list, profiles, jacobians, instrument, profileNumber, wavenumbers, ofWhat):
    ...
    if(ofWhat.lower()=='ozone'):
        for i in range(jacobians.shape[0]):
            jacobians[i,:] = jacobians[i,:]*profiles.O3[int(profileNumber)-1,:]
    elif(ofWhat.lower()=='water'):
         for i in range(jacobians.shape[0]):
            jacobians[i,:] = jacobians[i,:]*profiles.Q[int(profileNumber)-1,:]
    elif(ofWhat.lower()=='temperature'):
          for i in range(jacobians.shape[0]):
            jacobians[i,:] = jacobians[i,:]*profiles.T[int(profileNumber)-1,:]
    ...
    print(profileNumber)
    #plt.pcolor(np.arange(wf.shape[0]), myProfiles.P[0,0:99], wf[:,0:99].T, norm = LogNorm( vmin = wf[:,0:99].min(), vmax = 0.3 ))#vmax = wf[:,0:99].max() ) )
    plt.pcolor(np.arange(len(chan_list)+1), myProfiles.P[int(profileNumber)-1,:], jacSub[:,:].T ) 
    plt.colorbar()
    plt.ylabel('Pressure [hPa]')
    plt.xlabel('Instrument Channel')
    plt.gca().set_yscale('log')
    ...
    plt.close()
    print((profileNumber, myProfiles.O3[int(profileNumber)-1,:].sum()))
    print(('profile number, ozone value (units?)', profileNumber, myProfiles.O3[int(profileNumber)-1,:]))
...
def plotProfilesO3(profiles):
    ...
    #print a sample 03 profile
    print('O3 profile!')
    for j in range(profiles.O3[0,:].shape[0]):
        print(j, profiles.P[0,j], profiles.O3[0,j])
    print(('pressureLevels', profiles.P[0,:]))
    print((profiles.O3[0,:]))
    print('End O3 profile')
    plt.gca().set_yscale('log')
    plt.gca().invert_yaxis()
    plt.yticks(np.array([1000.0, 100.0, 10.0, 1.0, 0.1]),['1000.0','100.0','10.0','1.0','0.1'])
    legendList = []
...
if __name__ == '__main__':
    ...
    """
    chan_list_fsr  = [614, 626, 646, 648]#fsrH5['idxEcmwfOzoneInInstrument']
    chan_list_cris = [607, 626, 650] #crisH5['idxEcmwfOzoneInInstrument'] 
    chan_list_iasi = [1463, 1479, 1483, 1536, 1579, 1583, 1643, 1671]#iasiH5['idxEcmwfOzoneInInstrument']
    chan_list_airs = [1048, 1088, 1104, 1111, 1115, 1116, 1120 ]#airsH5['idxEcmwfOzoneInInstrument']
    chan_list_fsr  = [614, 626, 638, 659] #fsrH5['idxEcmwfOzoneInInstrument']
    chan_list_cris = [577, 607, 626, 650, 667] #crisH5['idxEcmwfOzoneInInstrument'] 
    chan_list_iasi = [1479, 1579, 1694, 1606, 1643, 1536, 1585] #iasiH5['idxEcmwfOzoneInInstrument']
    chan_list_airs = [1024, 1012, 1111, 1120, 1104, 1116, 1115, 1088] #airsH5['idxEcmwfOzoneInInstrument']
    """
    chan_list_fsr  = np.asarray(fsrH5['geosAssimilated']).tolist()
    chan_list_cris = np.asarray(crisH5['geosAssimilated']).tolist()
    chan_list_iasi = np.asarray(iasiH5['geosAssimilated']).tolist()
    chan_list_airs = np.asarray(airsH5['geosAssimilated']).tolist()
